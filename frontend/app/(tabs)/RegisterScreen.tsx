    ...
    >
      ...
      <View style={[styles.inputWrap, { borderColor }]}>
        <Ionicons
          name={field.icon}
          size={17}
          color={hasError ? '#EF4444' : focused ? Colors.primary : Colors.textMuted}
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.errors) {
          setErrors({
            fullName:        data.errors.name?.[0]                  ?? '',
            email:           data.errors.email?.[0]                 ?? '',
            phone:           data.errors.phone_number?.[0]          ?? '',
            plate:           data.errors.plate_number?.[0]          ?? '',
...
const styles = StyleSheet.create({
  ...
  dotDone: { width: 16, backgroundColor: Colors.accent ?? '#2ECC71' },
  dotInactive: { width: 16, backgroundColor: 'rgba(255,255,255,0.25)' },
  ...
  signInPrompt: { fontSize: 14, color: Colors.textSecondary },
